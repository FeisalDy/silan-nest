...
const LOCALIZATION: Record<string, { untitled: string; novel: string }> = {
  en: { untitled: 'Untitled', novel: 'Untitled Novel' }, 'zh-cn': { untitled: '无标题', novel: '无标题小说' }, zh: { untitled: '无标题', novel: '无标题小说' }, // Base fallback for any other Chinese region
  ja: { untitled: '無題', novel: '無題の小説' }, es: { untitled: 'Sin título', novel: 'Novela sin título' },
};
...
export class NovelTitleGenerator {
  static generate(options: {
    fileName?: string; firstChapterTitle?: string; languageCode?: Lang | string; // 2. Allow the Lang enum or string
  }): string {
    ...
    const t = LOCALIZATION[langInput] || LOCALIZATION[baseLang] || LOCALIZATION.en;
