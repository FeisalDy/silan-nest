...
export class NovelImportProcessor extends WorkerHost {
  ...
  async process(job: Job<NovelImportJobPayload>): Promise<void> {
    ...
    await this.dataSource.transaction(async (manager) => {
      ...
      if (parsedNovel.author) {
        author = await this.findOrCreateAuthor(manager, parsedNovel.author, parsedNovel.languageCode);
      }

      const novel = manager.create(Novel, {
        status: parsedNovel.status, authorId: author?.id ?? null,
      });
      ...
      await manager.save(manager.create(NovelTranslation, {
        novelId: savedNovel.id, languageCode: parsedNovel.languageCode, title: parsedNovel.title!, synopsis: parsedNovel.synopsis, slug, isDefault: true,
      }));

      for (const parsedChapter of parsedNovel.chapters) {
        const chapter = await manager.save(manager.create(Chapter, {
          novelId: savedNovel.id, chapterNumber: parsedChapter.chapterNumber, chapterSubNumber: parsedChapter.chapterSubNumber, volumeNumber: parsedChapter.volumeNumber,
        }));

        await manager.save(manager.create(ChapterTranslation, {
          chapterId: chapter.id, languageCode: parsedNovel.languageCode, title: parsedChapter.title || null, content: parsedChapter.content, isDefault: true,
        }));
      }
    });

    this.logger.log(`Import complete for "${parsedNovel.title}" — ${parsedNovel.chapters.length} chapter(s) saved.`);
  }
   ...
   */
  private async findOrCreateAuthor(manager: EntityManager, name: string, languageCode: string): Promise<Author> {
    ...
    await manager.save(manager.create(AuthorTranslation, {
      authorId: author.id, languageCode, name: normalized, isDefault: true,
    }));
